...
Please make sure not to publish information from Twitter you are not allowed to and to comply in strict accordance with the [twitter developer terms](https://developer.twitter.com/en/developer-terms/more-on-restricted-use-cases)! The example rtweet data in this article only contains the tweets of three dummy accounts I created. But probably you're not allowed to publish this for the `main_status_id` of any other tweet.
...
The generated graph object consists of nodes representing tweets and users, as well as edges that can be replies & quotes (connecting tweets), "by" connecting the author to his tweet, "like" connecting the user to the tweet he liked: 
...
We can generate a ggraph object and add profile pictures to the user nodes with the following function: 

```{r ggraph, eval=TRUE, fig.width=7, fig.height=6}

g1 <- add_profile_pics_to_tree_ggraph(
  g,
  rtweettree:::get_profile_pic_df(bind_rows(df_tls, df_favs, df_main_status))
)
# without profile picture, it would be:
# g1 <- g %>% ggraph()
# 
