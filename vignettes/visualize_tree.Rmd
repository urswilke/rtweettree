...
```{r tbl_graph, eval=TRUE}
g <-
  create_tweet_tbl_graph(list(df_main_status, df_tree, df_tls, df_favs, df_retweets))
```
...
* "like" connecting the user to the tweet he liked. 
...
```{r ggraph, fig.width=8, fig.height=6}
df_profile_pic <- rtweettree:::get_profile_pic_df(bind_rows(df_tls, df_favs, df_main_status))
```
```{r, eval=TRUE, include=FALSE}
df_profile_pic$profile_image_url <- 1:3 %>% purrr::map_chr(~system.file("profile_pics", paste0(.x, ".png"), package = "rtweettree"))
...
```{r df_profile_pic_save, include=FALSE}
df_profile_pic <- df_profile_pic %>%
    dplyr::mutate(
      img = profile_image_url %>%
        purrr::map(magick::image_read)
    )
usethis::use_data(df_profile_pic, overwrite = TRUE)
```
